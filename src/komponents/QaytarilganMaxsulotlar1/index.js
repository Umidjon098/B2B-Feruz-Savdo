...
const  _QaytarilganMaxsulotlar = () => {
    ...
    const getQaytganMaxsulotlar = () => {
        axios.get(`order/provider-failed-product-list/?limit=${limit}&offset=${(offset-1)*limit}`,{
            params: {
    ...
    return (
        ...
        <Title padding="" text="Xaridorlar tomonidan qaytarilgan maxsulotlar"/>
...
const ListItemOne = (props) => {
    ...
    const [sellOrder, setSellOrder] = React.useState(props.qaytarilgan)
    ...
    const updateProduct = (_id) => {
        axios.put(`order/provider-failed-product-detail/${_id}/`,{
            buy_order: qaytgan.buy_order.id,
            failed_status: turi,
    ...
    const deleteProduct = (_id) => {
        axios.delete(`order/provider-failed-product-detail/${_id}/`).then((res)=>{
            if(res.status == 204){
                setBormi(false);
    ...
    return(
        <tr>
            ...
            <td>
                <>
                    {sellOrder.buy_order.product.name}
                </>
            </td>
            <td>
                {
...
export default observer(_QaytarilganMaxsulotlar);